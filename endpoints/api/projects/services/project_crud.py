...
def create_project(user, data):
    ...
    if project.product:
        try:
            ...
            enriched = calculate_project_metrics(project.product.name, calc_input)
    ...
    if project.project_attributes and isinstance(project.project_attributes.get("wg_data"), dict):
        ...
        if wg_data.get("project_number") and wg_data.get("tenant"):
            enriched_wg_data = enrich_wg_data(wg_data)
    ...
    if (data.get("product_id") == 2 and data.get("submitToWG") == True):
        ...
        submit_shade_sail_to_workguru(project, data, wg_client_id, wg_name)
...
def hard_delete_project(user, project_id):
    """Permanently delete a project and all its products from the database."""
    # Authorization check - Admin only
    if user.role != "admin":
        raise ValueError("Unauthorized: Only admins can permanently delete projects")

    # Proceed with deletion regardless of 'deleted' flag (can delete active or soft-deleted)
    project = Project.query.get(project_id)
    if not project:
        raise ValueError("Project not found")

    # Delete all associated project products first
    ProjectProduct.query.filter_by(project_id=project_id).delete()
