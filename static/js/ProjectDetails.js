...
const defaultSchema = {
  Materials: [
    { type: 'row', description: 'Fabric', quantity: "data.calculated?.nestData?.total_width ? data.calculated.nestData.total_width / 1000 : 0", unitCost: 13.33 },
    { type: 'row', description: 'Zip', quantity: "2 * (data.attributes?.quantity || 0)", unitCost: 0.65 },
    { type: 'row', description: 'Thread', quantity: "2", unitCost: 0.03 },
    { type: 'subtotal', label: 'Total Materials', key : 'materialsTotal'}
  ],
  Labour: [
    { type: 'row', description: 'Design', quantity: 0.5, unitCost: 55 },
    { type: 'row', description: 'Cutting/Plotting', quantity: "0.4", unitCost: 55 },
    { type: 'subtotal', label: 'Total Labour', key : 'labourTotal'}
  ],
  Summary: [
    { type: 'calc', key: 'totalCostFabrication', label: 'Total Cost Fabrication', expr: 'materialsTotal + labourTotal' },
    { type: 'input', label: 'Contingencies %', key: 'contingencyPercent', default: 3 },
    { type: 'calc', key: 'contingencyAmount', label: 'Contingency Amount', expr: 'baseCost * contingencyPercent / 100' },
    { type: 'input', label: 'Gross Margin %', key: 'marginPercent', default: 45 },
    { type: 'calc', key: 'marginAmount', label: 'Margin Amount', expr: '(baseCost + contingencyAmount) * marginPercent / 100' },
    { type: 'calc', key: 'suggestedPrice', label: 'Suggested Price', expr: 'baseCost + contingencyAmount + marginAmount' }
  ]
};
